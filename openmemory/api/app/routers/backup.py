...
from app.models import User, App, Memory, MemoryState, Category, memory_categories, MemoryStatusHistory, AccessControl
...
def _export_sqlite(db: Session, req: ExportRequest) -> Dict[str, Any]:
    ...
    history = (
        db.query(MemoryStatusHistory).filter(MemoryStatusHistory.memory_id.in_(memory_ids)).all() if memory_ids else []
    )

    acls = (
        db.query(AccessControl)
        .filter(AccessControl.subject_type == "app", AccessControl.subject_id.in_(app_ids) if app_ids else False)
        .all()
        if app_ids
        else []
    )
        ...
        "memories": [
            {
                ...
                "category_ids": [
                    str(c.id) for c in m.categories
